...
//const localApi = `${environment.configuration.api_url}`
...
const outputInitialSettings = {
  ...
  //host: localApi.replace("http://","").replace("https://","").replace("/api/v1",""),
  //schemes: (environment.configuration.environment=='development')?['http']:['https'],
  schemes: ['http'],
  info: {
  ...
  securityDefinitions:{
    apiKeyAuth:{
        ...
        description: "dont forget do add Bearer before the token" 
    }
  }
}
